...
def main():
    ...
    if len(os.getenv('app_key')) not in (32, 48) or len(os.getenv('app_secret')) not in (16, 64): # type: ignore
        raise Exception("Add you app key and app secret to the .env file.")

    # set logging level
    logging.basicConfig(level=logging.INFO)

    client = schwabdev.Client(os.getenv('app_key'), os.getenv('app_secret'), os.getenv('callback_url')) # type: ignore
